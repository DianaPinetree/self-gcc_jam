...
public class GameController : MonoBehaviour
{
    ...
    float enemySpawnTimerCD = 30f;
    private float enemySpawnTimerCDLow = 4f;
    private float spawnRateCapTime = 13f * 60f;
    ...
    private float updatesCD = 0.2f;
    private int batchIndex = 0;
    private float updatesPerSecond;
    private float updatesPerFrame;
    ...
    // For bullets
    [HideInInspector] public Dictionary<int, HashSet<ProjectileBehaviour>> bulletSpatialGroups = new Dictionary<int, HashSet<ProjectileBehaviour>>();
    [HideInInspector] public Dictionary<int, HashSet<PickUp>> pickupsSpacialGroup = new Dictionary<int, HashSet<PickUp>>();
    ...
    public HashSet<PowerUp> CollectedPowerUps = new HashSet<PowerUp>();
    //! MIN HEAP FOR BATCH //
    ...
    void Start()
    {
        currentCD = enemySpawnTimerCD;
        updatesPerSecond = 50 / updatesCD;
        ...
        for (int i = 0; i < numberOfPartitions; i++)
        {
            ...
            bulletSpatialGroups.Add(i, new HashSet<ProjectileBehaviour>());
            pickupsSpacialGroup.Add(i, new HashSet<PickUp>());
        }
    ...
    void FixedUpdate() // 50 frames per second
    {
        ...
        updatesPerFrame = updatesPerSecond * Time.fixedDeltaTime;
        runLogicTimer += Time.deltaTime;
        updateAccumulator = Mathf.Min(updateAccumulator + updatesPerFrame, 50);
        ...
        {
            batchIndex = (batchIndex + 1) % 50;
            RunBatchLogic(batchIndex); // runLogicTimer is the batchID, for that set of enemies
        }

        updateAccumulator -= updateCount;
    }

    void RunOnceASecondLogicForAllBullets()
    {
        // Debug.Log("# of bullets updating: " + bulletSpatialGroups.SelectMany(x => x.Value).Count());
        foreach (ProjectileBehaviour bullet in bulletSpatialGroups.SelectMany(x => x.Value).ToList())
        {
    ...
    void SpawnEnemies()
    {
        ...
        currentCD = Mathf.Lerp(enemySpawnTimerCD, enemySpawnTimerCDLow, elapsed / spawnRateCapTime);
        if (enemySpawnTimer > currentCD && enemyHolder.childCount < maxEnemyCount)
        {
            for (int i = 0; i < 10; i++)
            {
    ...
    public void FastTrackWave()
    {
        enemySpawnTimer = currentCD + 1;
    }

    public void SpawnEnemy(GameObject enemyPF)
    {
